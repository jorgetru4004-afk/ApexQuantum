...
const COINBASE_SECRET = process.env.COINBASE_API_SECRET  || '';
...
// ── PRICE SIMULATION ──
// CoinGecko ID map — maps our symbols to CoinGecko IDs
const COINGECKO_IDS = {
  BTC:'bitcoin', ETH:'ethereum', SOL:'solana', DOGE:'dogecoin',
  PEPE:'pepe', BONK:'bonk', WIF:'dogwifcoin', AVAX:'avalanche-2',
  MATIC:'matic-network', LINK:'chainlink', DOT:'polkadot',
  ADA:'cardano', XRP:'ripple', SHIB:'shiba-inu', LTC:'litecoin',
  AAVE:'aave', UNI:'uniswap', ONDO:'ondo-finance', TAO:'bittensor',
  HYPE:'hyperliquid', PENDLE:'pendle', VIRTUAL:'virtual-protocol',
  JUP:'jupiter-exchange-solana', RNDR:'render-token',
  SUI:'sui', INJ:'injective-protocol', TON:'the-open-network',
};

let lastCoinGeckoFetch = 0;
let coinGeckoPrices = {}; // cache real prices

async function fetchCoinGeckoPrices() {
  try {
    // Build list of CoinGecko IDs for currently tracked coins
    const ids = coins.map(s => COINGECKO_IDS[s]).filter(Boolean).join(',');
    if (!ids) return;
    ...
    coins.forEach(sym => {
      ...
      coinGeckoPrices[sym] = {
        price:     data[id].usd,
        change24:  data[id].usd_24h_change || 0,
        vol24h:    data[id].usd_24h_vol    || 0,
      };
    });
    lastCoinGeckoFetch = Date.now();
    console.log(`💰 CoinGecko prices updated — BTC: $${coinGeckoPrices['BTC']?.price?.toFixed(2) || 'N/A'}`);
  } catch(e) {
    console.log('⚠️ CoinGecko fetch failed — using last known prices');
  }
}
...
function tickPrices() {
  coins.forEach(sym => {
    ...
    // Use real CoinGecko price if available, otherwise simulate small movements
    const real = coinGeckoPrices[sym];
    if (real && real.price) {
      // Apply tiny realistic micro-movement on top of real price
      const micro = (Math.random()-0.5) * 0.0002;
      c.price    = parseFloat((real.price * (1 + micro)).toFixed(8));
      c.change24 = parseFloat(real.change24.toFixed(2));
      if (real.vol24h) vols24h[sym] = real.vol24h;
    } else {
      // Fallback simulation if CoinGecko unavailable
...
// Keep simulatePrices as alias for compatibility
function simulatePrices() { tickPrices(); }
...
function runBotLogic(sym) {
  ...
  if (b.pos > 0) {
    ...
    if (pctFromEntry <= -SETTINGS.stopLoss || pctFromEntry >= SETTINGS.takeProfit || trailingHit) {
      ...
      saveState();
      if (coinbaseConnected) placeCoinbaseOrder(sym, 'sell', b.allocated||SETTINGS.budget).catch(()=>{});
...
async function testCoinbase() {
  if (!COINBASE_KEY || !COINBASE_SECRET) {
    console.log('⚠️ Coinbase: No API credentials — running in simulation mode');
    return;
  }
  try {
    ...
    const signature = crypto.createHmac('sha256', COINBASE_SECRET)
    ...
    const balance  = usd ? parseFloat(usd.available_balance?.value || 0).toFixed(2) : 'N/A';
    console.log(`✅ Coinbase connected — $${balance} USD available`);
    sendDiscordAlert(
      `✅ **Coinbase Connected**
` +
      `💰 USD Balance: $${balance}
` +
      `📊 Mode: LIVE CRYPTO TRADING`
    );
    broadcast('COINBASE_STATUS', { connected: true, balance, paper: false });
  } catch (e) {
    console.log('⚠️ Coinbase not connected:', e.message);
    coinbaseConnected = false;
  }
}

async function placeCoinbaseOrder(sym, side, dollars) {
  if (!coinbaseConnected || !COINBASE_KEY || !COINBASE_SECRET) return;
  try {
    const timestamp   = Math.floor(Date.now() / 1000).toString();
    const method      = 'POST';
    const path        = '/api/v3/brokerage/orders';
    const productId   = sym + '-USD'; // e.g. BTC-USD, ETH-USD
    const body        = JSON.stringify({
      client_order_id: `apex_${Date.now()}`,
      product_id:      productId,
      side:            side.toUpperCase(), // BUY or SELL
      order_configuration: {
        market_market_ioc: {
          quote_size: dollars.toFixed(2) // spend/receive this many USD
        }
      }
    });
    ...
    console.log(`📋 Coinbase ${side} $${dollars} of ${sym} — order ${resp.data?.success_response?.order_id}`);
  } catch (e) {
    console.error('Coinbase order error:', e.message);
  }
}
...
// ── CRON SCHEDULE ──
setInterval(tickPrices, 3000);              // Price tick every 3s using real prices
setInterval(fetchCoinGeckoPrices, 30000);   // Refresh real prices from CoinGecko every 30s
